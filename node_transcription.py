...
class OpenRouterTranscriptionNode:
    ...
    def fetch_openrouter_models(cls):
        """
        Fetches STT model IDs from the OpenRouter API, filtered to models
        whose input modality includes audio (e.g. "audio->text").

        Falls back to the hardcoded Whisper list on any error.
        """
        current_time = time.time()
        if cls.models_cache is None or (current_time - cls.last_fetch_time > cls.cache_duration):
            try:
                raw, _ = shared.fetch_all_models_raw()
                models = shared.filter_models_by_input(raw, "audio")
